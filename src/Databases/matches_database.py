...
def complete_match(challenge_id, winner_id, challenger_score, challengee_score):
    try:
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE challenges SET status = 'completed' WHERE id = %s;",
                    (challenge_id,),
                )
                cur.execute(
                    "INSERT INTO matches (challenge_id, winner_id, challenger_score, challengee_score) VALUES (%s, %s, %s, %s) RETURNING id;",
                    (challenge_id, winner_id, challenger_score, challengee_score),
                )
                conn.commit()
                return "success"
    except (Exception, psycopg2.DatabaseError) as error:
